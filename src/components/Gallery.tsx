      ...
      <div className="container-tight">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 text-brand-navy/70">
            Kitchens, floors, bathrooms, bedrooms, and carpets — organized so you can see exactly
            what we deliver.
          </p>
        </div>
        ...
        <LayoutGroup>
          <motion.div
            layout
            className="mt-10 grid auto-rows-[160px] grid-cols-2 gap-3 sm:auto-rows-[180px] sm:gap-4 md:auto-rows-[200px] md:grid-cols-3 lg:grid-cols-4"
          >
            <AnimatePresence mode="popLayout">
              {items.map((img, idx) => {
                const featured = !!img.feature;
                const span = featured
                ...
                return (
                  <motion.button
                    ...
                    className={`group relative block overflow-hidden rounded-2xl bg-brand-soft shadow-card ring-1 ring-brand-navy/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-teal ${span}`}
                  >
                    <Image
                      src={img.src}
                      alt={img.alt}
                    ...
                    <div className="pointer-events-none absolute inset-x-0 bottom-0 translate-y-2 p-3 text-left text-white opacity-0 transition group-hover:translate-y-0 group-hover:opacity-100">
                      <p className="text-sm font-medium leading-snug">{img.caption}</p>
                    </div>
                  </motion.button>
                );
              })}
