      ...
      <div className="container-wide">
        <div className="text-center max-w-2xl mx-auto">
          ...
          <p className="mt-4 text-brand-slate text-base sm:text-lg">
            Charleston, Summerville, Goose Creek, Mount Pleasant, Folly Beach, Kiawah, Isle of Palms, and
            every Lowcountry community in between.
          </p>
        </div>

        <div className="mt-10 rounded-3xl bg-aqua-wash p-6 sm:p-10 ring-1 ring-brand-navy/5">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {locations.map((loc) => (
              <Link
                key={loc.slug}
                href={`/service-areas/${loc.slug}`}
                className="group flex items-center gap-2 rounded-xl bg-white/80 hover:bg-white px-3 py-2.5 text-sm font-semibold text-brand-navy ring-1 ring-brand-navy/5 hover:ring-brand-aqua/40 hover:shadow-card transition"
              >
